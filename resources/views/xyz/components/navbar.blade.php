<nav class="navbar navbar-expand-lg bg-primary-subtle fixed-top">
    <div class="container">
        <a class="navbar-brand fw-bold" href="">PT. XYZ</a>
            ...
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a @if ($data['title'] === 'Home') class="nav-link active" @else class="nav-link" @endif
                        aria-current="page" href="/xyz/home">Home</a>
                </li>
                @if (Auth::user()->role == 1)
                    <li class="nav-item">
                        <a @if ($data['title'] === 'Register User') class="nav-link active" @else class="nav-link" @endif
                            aria-current="page" href="/xyz/admin/registerUser">Register User</a>
                    </li>
                @endif
                <li class="nav-item">
                    <a @if ($data['title'] === 'Data User') class="nav-link active" @else class="nav-link" @endif
                        class="nav-link" aria-current="page" href="/xyz/dataUser">Data User</a>
                </li>
            </ul>
